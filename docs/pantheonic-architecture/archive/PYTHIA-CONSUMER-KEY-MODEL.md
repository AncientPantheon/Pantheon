...
> **⚠ SUPERSEDED (2026-07-31).** This single-Apollo-key, baked-public-key-header,
> one-time-activation model was never built. What actually shipped — grounded against
> the *live* on-chain `PYTHIA.pact` module, not a stale local checkout — is a
> **dual-Apollo (Standard/Smart) pair**, a **repeating 3-hour ephemeral-secret**
> challenge/sign/verify round trip, and module/table names (`PYTHIA`,
> `PYTHIA|S|ApiKey`, `PYTHIA|S|DualLink`) that do not match this doc's `APIARY`
> sketch. See **[`organs/06-pythia-client-wire-in.md`](../organs/06-pythia-client-wire-in.md)**
> for the current, shipped protocol. Kept here for historical lineage only — do not
> build against this doc.
