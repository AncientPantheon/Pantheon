...
> **Re-derive seed accounts the SAME seedType-aware way Codex RECORDED them (learned live in Pythia, 2026-08-03).** When your `KeyResolver.getKeyPairByPublicKey` re-derives a key from an HD-wallet **seed** account (as opposed to a directly-held pure/ouro keypair), the derivation MUST match how Codex stored that account's public key — and Codex routes on the seed's `seedType`: a `koala` seed (24-word BIP39) uses SLIP-10 Ed25519 (`m'/44'/626'/idx'` via `kadenaMnemonicToSeed`+`kadenaGenKeypairFromSeed`), while a `chainweaver`/`eckowallet` seed (12-word) uses Chainweaver's BIP32-Ed25519 (WASM) scheme (`kadenaMnemonicToRootKeypair`+`kadenaGenKeypair` from `@stoachain/kadena-stoic-legacy/hd-wallet/chainweaver`). Pythia's resolver first used the koala path for EVERY seed, so a chainweaver operator seed re-derived a different key at the same index and the resolver's own safety guard refused to sign (`seed "…" derived a different key at index N than the codex recorded`). Both derivations produce a **password-independent** public key, so re-deriving with a fresh transient password reproduces exactly what Codex stored — the ONLY thing you must get right is branching on `seedType`. Keep the wrong-key guard (a re-derived pubkey ≠ the recorded one must refuse to sign — never sign with an unverified key). Reference implementation: `constructors/Pythia/apps/pythia/src/automaton/khronoton/keyResolver.ts` (`fromSeedAccount`, seedType branch) + `keyResolver.test.ts` (real koala + chainweaver seeds). Pure/ouro accounts are unaffected — they hold the secret directly and skip re-derivation entirely.
